...
while input_cam.isOpened():         # Пока объект захвата открыт
    ...
    Новые значения каждого пикселя - средневзвешанное значение окрестности этого пиксела 
    Значения этого распределения используются для построения матрицы свёртки, 
    которая применяется к исходному изображению. 
    Свёртка — это операция, показывающая «схожесть» 
    одной функции с отражённой и сдвинутой копией другой.
    Свёртка — это операция вычисления нового значения заданного пикселя, 
    при которой учитываются значения окружающих его соседних пикселей.
    Главным элементом свёртки является ядро свёртки — это матрица 

    береём ядро, загоряем координаты его центральной точки в формулу
    суммируем результаты и загоняем в центр, 
    так делаем для каждого ядра
    потом выполняем свёртку
    """
